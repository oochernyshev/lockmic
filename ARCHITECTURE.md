...
**Owner:** [WIXEE.AI](https://wixee.ai) · **License:** MIT · **Current version:** 1.4.47
...
| **1.4.12** | Lower CPU while recording; drop leftover session-folder mix | Done |
| **1.4.13** | Hours on long recordings; mix tags and LockMic cover art | Done |
| **1.4.14** | LED meters; clickable device rows in the recording monitor | Done |
| **1.4.15** | Gapless live mix with playable 30-second AAC checkpoints | Done |
| **1.4.16** | Appearance setting (system/light/dark) | Done |
| **1.4.17** | Lighter recording CPU; remember devices; per-output meters; missed-source warning | Done |
| **1.4.18** | Faster relaunch: wait only if the other instance is wrapping a recording | Done |
| **1.4.19** | Dim unselected monitor rows; gray theme-aware meters; shortcut toggles window | Done |
| **1.4.20** | Record-all playback stays selected and actually captures every output | Done |
| **1.4.21** | System-tap playback capture stays continuous across output-device switches | Done |
| **1.4.22** | Floating HUD hide works on identical monitors; Hide on All Displays | Done |
| **1.4.23** | Recording monitor Low quality badge when a Bluetooth headset mic is open | Done |
...
| **1.4.28** | Silence auto-stop ignores clicks/noise; cancellable countdown in the last 30 seconds | Done |
| **1.4.29** | Device disconnect fallback; smoothed energy silence detection and HUD warning; startup icon fix | Done |
| **1.4.30** | Live silence graph and clickable timeout badge; safe timeout changes during recording | Done |
...
| **1.4.36** | one-command GitHub Homebrew and website release | Done |
| **1.4.37** | Default, All, or Selection for recorded outputs | Done |
| **1.4.38** | System mix vs per-device output recording | Done |
...
| **1.4.43** | Render the floating HUD with real Liquid Glass on macOS 26 | Done |
| **1.4.44** | Add guided onboarding and clearer first-run setup | Done |
| **1.4.45** | Fix a crash when an audio device fails to start or disconnects | Done |
| **1.4.46** | Use the correct update channel for App Store and direct installs | Done |
| **1.4.47** | Developer ID signing and notarized direct downloads | Done |
